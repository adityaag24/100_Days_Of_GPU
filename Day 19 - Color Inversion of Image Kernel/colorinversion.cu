...
__global__ void invertColor(unsigned char *inputImage, int width, int height) {
    ...
    if (row < height && col < width) {
        ...
        int rgb = idx * 3;
        inputImage[rgb + 0] = (255 - inputImage[rgb + 0]);
        inputImage[rgb + 1] = (255 - inputImage[rgb + 1]);
        inputImage[rgb + 2] = (255 - inputImage[rgb + 2]);
    }
}
int main() {
    ...
    invertColor<<<grid, block>>>(d_inputImage, width, height);
    CHECK_CUDA_ERROR(cudaMemcpy(h_outputImage, d_inputImage, width * height * channels * sizeof(unsigned char), cudaMemcpyDeviceToHost));
